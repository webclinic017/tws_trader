...
def K_below_D(stoch_of_company_now):
	companies_with_crossing_KD = set()
	for company in stoch_of_company_now:
		if stoch_of_company_now[company][0] < stoch_of_company_now[company][1]:	# %K < %D
			companies_with_crossing_KD.add(company)
	return companies_with_crossing_KD
...
def main(set_of_companies):
	...
	# print(f'Here is {len(interesting_to_BUY_companies)} companies you may BUY:')
	# for company in interesting_to_BUY_companies:
	# 	print(company, companies[company])
	# print('\n')
	# print(f'Here is {len(interesting_to_SELL_companies)} companies you may SELL:')
...
if __name__ == '__main__':
	companies = get_companies_with_data()
	main(companies)
