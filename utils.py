...
def print_loading(done_number, total_number, company):
	...
	if done_number < total_number:
		print(f'  {int(round(percentage * 3.33, 0))}% |' + '█' * percentage + ' ' * (30 - percentage) + '|',
		      f'{done_number}/{total_number} Requesting data for {company}', ' ' * 5, end='\r')
	else:
		time_now = time.strftime("%m/%d/%Y %I:%M %p", time.gmtime())
		print(f' {int(round(percentage * 3.33, 0))}% |' + '█' * percentage + ' ' * (30 - percentage) + '|',
		      f'{done_number}/{total_number} Updating complete!', time_now)
...
def max_drawdown_calculate(price_data):
	max_drawdown = 0
	for i in range(len(price_data)):
		capital = price_data[:len(price_data) - i]
		max_capital = max(tuple(x['Capital'] for x in capital))
		max_capital_date_index = None
		for row in capital:
			if max_capital == row['Capital']:
				max_capital_date_index = capital.index(row)
		if max_capital == capital[-1]['Capital']:
			min_capital_since_max_capital = max_capital
		else:
			min_capital_since_max_capital = min(tuple(x['Capital'] for x in capital[max_capital_date_index + 1:]))
...
def update_price_data(company, bar_size):
	...
	data = yf.Ticker(company).history(interval=y_interval[bar_size]).iloc[:,
	       :-2]  # excluding Dividends and Stock Splits
